...
const Dashboard: React.FC<DashboardProps> = ({ onBrowse, onItemClick }) => {
  ...
  return (
    <div className="space-y-10 pb-10">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl font-display tracking-tight text-emerald-950">Overview</h1>
          <p className="text-slate-400 text-sm mt-1 font-medium">Verified circular stock signals.</p>
        </div>
        <div className="flex gap-3 bg-white/60 p-1 rounded-full border border-white">
          {(['All', 'Buy', 'Sell'] as const).map((r) => (
            <button 
              ...
              className={`px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${filterRole === r ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400'}`}
            >
              {r}
            </button>
          ))}
      ...
      >
          ...
          {filteredListings.map((listing) => (
            ...
            >
              <div className="flex gap-6 items-start">
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start mb-2">
                    <span className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest ${listing.tradeType === 'Sell' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'}`}>
                      {listing.tradeType}
                    </span>
                    {listing.isVerified && <CheckCircle2 size={14} className="text-emerald-500" />}
                  </div>
                  <h4 className="text-xl font-display text-slate-900 group-hover:text-emerald-600 transition-colors">{listing.materialType}</h4>
              ...
              <div className="mt-6 pt-6 border-t border-slate-50 flex justify-between items-center">
                <div>
                  <p className="text-[9px] uppercase tracking-widest text-slate-400 font-bold mb-1">Approx. Value</p>
                  <p className="text-xl font-display font-medium text-slate-900">₹{listing.pricePerUnit.toLocaleString()}<span className="text-[10px] text-slate-300 ml-1">/MT</span></p>
                </div>
                <button className="bg-slate-900 text-white p-3 rounded-xl group-hover:bg-emerald-600 transition-all">
        ...
        {filteredListings.length === 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="col-span-full py-20 text-center">
            ...
            <p className="text-slate-400 italic text-sm">No operational signals match your parameters.</p>
          </motion.div>
        )}
      </motion.div>
    </div>
